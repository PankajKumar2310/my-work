...
export class ChatbotAI {
    ...
    getGreetingResponse() {
        const responses = [
            ...
            `Hello! Welcome to Pankaj's portfolio ! I can tell you about his projects, technical skills, work experience, or how to get in touch. What interests you most?`,
            `Hi! Great to meet you! I'm here to answer any questions about pankaj's background, projects, or expertise. Feel free to ask me anything!`
        ];
        return this.getRandomResponse(responses);
    }
    isPersonalInfoQuery(input) {
        const keywords = [
            'who are you', 'who is pankaj', 'about pankaj', 'pankaj intro', 'introduction',
            'tell me about yourself', 'your intro', 'pankaj meena', 'pankaj information',
            'brief about pankaj', 'what do you do', 'pankaj kya karta hai', 'pankaj kaun hai'
        ];
    ...
    getSkillsResponse(input) {
        ...
        const name = this.normalizeName(input); // handles "Radhe" or "Pankaj"
    ...
    getInitialProjectPrompt() {
        return `Pankaj has worked on several exciting projects! Would you like to hear about them one by one? Just say "yes", "go ahead", or name a specific project like "project one" to begin.`;
    }
    ...
    getProjectByIndex(index) {
        ...
        if (index >= 0 && index < projects.length) {
            ...
            return `Sorry, Pankaj doesn't have that many projects. Try asking about "project one" or say "next" to continue.`;
        }
    }
    ...
    getContactResponse(input = '') {
        ...
        if (lowerInput.includes('whatsapp')) {
            return `📱 You can reach Pankaj on WhatsApp at: ${socialMedia.whatsapp}`;
        }

        if (lowerInput.includes('mobile') || lowerInput.includes('phone') || lowerInput.includes('call')) {
            return `📞 Pankaj’s mobile number is: ${personalInfo.phone}`;
        }

        if (lowerInput.includes('linkedin')) {
            return `🔗 Here’s Pankaj’s LinkedIn profile: ${socialMedia.linkedin}`;
        }

        if (lowerInput.includes('github')) {
            return `🧑‍💻 Pankaj’s GitHub: ${socialMedia.github}`;
        }

        if (lowerInput.includes('instagram')) {
            return `📸 Pankaj’s Instagram: ${socialMedia.instagram}`;
        }

        if (lowerInput.includes('mail') || lowerInput.includes('email')) {
            return `✉️ Pankaj’s email: ${personalInfo.email}`;
        }

        // If no specific platform is mentioned → show full contact info
        return `📬 You can reach Pankaj in the following ways:\n\n` +
            `📧 Email: ${personalInfo.email}\n` +
            `📱 Phone: ${personalInfo.phone}\n` +
            `💬 WhatsApp: ${socialMedia.whatsapp}\n` +
            `🔗 LinkedIn: ${socialMedia.linkedin}\n\n` +
            `Pankaj is currently ${availability.status.toLowerCase()} and typically ${availability.responseTime.toLowerCase()}. ${availability.preferredContact} is the best way to connect!`;
    }
    ...
    getEducationResponse(input = '') {
        ...
        if (lowerInput.includes('school')) {
            ...
            return `For schooling, Pankaj studied at ${edu.institution} and completed ${edu.level} in ${edu.year}. ${edu.details}`;
        }
    ...
    isCurrentActivityQuery(input) {
        return ['where is pankaj', 'what is pankaj doing', 'what’s pankaj up to', 'pankaj busy', 'can i talk to pankaj', 'pankaj available', 'pankaj free', 'pankaj now'].some(phrase => input.toLowerCase().includes(phrase));
    }
    ...
    getCurrentActivityResponse() {
        ...
        const getStatus = () => {
            ...
            const timeString = `Right now it's ${String(currentHourIST).padStart(2, '0')}:${String(currentMinuteIST).padStart(2, '0')} IST, so Pankaj is probably `;
        
            if (currentHourIST >= 23 || currentHourIST < 7) {
                return `${timeString}sleeping 😴. He usually rests between 2 PM and 7 AM.`;
            }
        ...
        getFreeTimeResponse() {
            ...
            if (isSunday) {
                return `Right now it's ${currentTimeFormatted}, and it's Sunday — Pankaj is probably ${currentActivity?.label || "relaxing"}. He’s free today, feel free to reach out or call him anytime 📞.`;
            }
        
            if (currentHourIST >= 20 && currentHourIST < 23) {
                return `Right now it's ${currentTimeFormatted}, Pankaj is probably ${currentActivity.label}. He’s usually free after 8 PM on working days, so you can reach out now ✅.`;
            }
        
            return `Right now it's ${currentTimeFormatted}, Pankaj is ${currentActivity?.label}. After this, he'll be ${nextActivity?.label}. On working days, he’s usually free between 8 PM to 11 PM. Shall I remind you then? 📅`;
        }
        ...
        getFollowupContactResponse() {
            return `🙌 Thank you! You can call or WhatsApp him on +91 8529817616— he’ll respond shortly!!`;
        }
        ...
        getInappropriateResponse() {
            const responses = [
                "This assistant is here to help you explore Pankaj’s professional background. Let’s please keep the conversation respectful 🙏.",
                "I'd appreciate if we keep the tone polite. If you’d like to learn about Pankaj’s skills or work, I’m here to assist.",
                "Inappropriate language isn’t allowed here. Please ask genuine questions if you’d like to know about Pankaj’s projects or experience.",
                "Let’s keep things clean and professional. I'm here to assist with Pankaj’s portfolio, not tolerate offensive language."
            ];
            return this.getRandomResponse(responses);
        }
        ...
        isRelationshipQuery(input) {
            const loveKeywords = [
                ...
                'pankaj ki girlfriend', 'radhe ki gf'
            ];
        ...
        getDefaultResponse() {
            const earlyResponses = [
                "Hmm, I didn’t quite catch that. You can ask me about Pankaj’s skills, projects, experience, or how to get in touch with him. What would you like to know?",
                "That's a good question! I'm here to help you learn more about Pankaj — his work, tech stack, past experiences, or even what he's currently up to 😊",
                "I'm Pankaj’s virtual assistant! Feel free to ask me anything about his background, projects, or how you can connect with him.",
                "I couldn’t find a clear match for that, but no worries! I can guide you through Pankaj’s education, technical strengths, or current availability. Try asking about one of those!"
            ];
        
            const laterResponses = [
                "Looks like we’ve covered quite a bit! If you have more questions about Pankaj, feel free to ask — I’m always here to help 🤗",
                "If you're wrapping up, it was great chatting with you! You can always reach out to Pankaj via the contact. 📧.",
                "That one’s a bit tricky, but I’ll keep improving! Meanwhile, I can still share more about Pankaj’s journey, skills, or projects.",
                "Not sure about that — but I can definitely tell you more about Pankaj’s profile, tech stack, or how to connect with him!"
            ];
